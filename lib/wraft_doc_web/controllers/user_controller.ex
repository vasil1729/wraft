defmodule WraftDocWeb.Api.V1.UserController do
  ...
  def signin(conn, params) do
    account_lookup_result = Account.find(params["email"])

    case account_lookup_result do
      %User{} -> :ok
      _ -> Bcrypt.no_user_verify()
    end

    with %User{} = user <- account_lookup_result,
      ...
      # We must map {:error, :invalid_email} from Account.find to {:error, :invalid}
      # to ensure the FallbackController returns the generic 404 message
      # "Your email-password combination doesn't match..." instead of "No user with this email.!"
      {:error, :invalid_email} -> {:error, :invalid}
      error -> error
    end
  end
  ...
  def generate_token(conn, params) do
    # Note: Returning generic success to prevent email enumeration
