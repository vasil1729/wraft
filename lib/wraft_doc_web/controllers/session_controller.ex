defmodule WraftDocWeb.SessionController do
  ...
  def create(conn, %{"session" => params}) do
    ...
    with %InternalUser{} = found_user <- user,
         true <- Bcrypt.verify_pass(params["password"], found_user.encrypted_password),
         false <- found_user.is_deactivated do
      conn
      |> put_session(:admin_id, found_user.id)
