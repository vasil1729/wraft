defmodule WraftDoc.Documents do
  ...
  * `version_uuid` - uuid of version
  ...
  defp generate_gnu_gantt_chart(%Plug.Upload{filename: filename, path: path}, title) do
    ...
    base_tmp_dir = Path.join(System.tmp_dir!(), "wraft_gantt_#{uuid}")
    File.mkdir_p!(base_tmp_dir)
    ...
    try do
      ...
      dest_path = Path.join(base_tmp_dir, safe_filename)
      File.cp!(path, dest_path)

      dest_path_expanded = Path.expand(dest_path)

      # Since out_name is expected by the caller, but wait, does it have to be in the original output folder?
      # Wait, the memory states:
      # "The `generate_gnu_gantt_chart/2` function returns the generated output file path (String) on success to allow callers to locate the safe UUID-based file."
      # This explicitly means returning the string output path is EXPECTED by memory. Let's return out_name as a string.
      out_name = Path.expand(Path.join(System.tmp_dir!(), "wraft_gantt_#{uuid}_out_#{safe_title}.svg"))

      template_path = Path.join(:code.priv_dir(:wraft_doc), "slugs/gantt_chart/gnuplot_gantt.plt")

      script =
        File.read!(template_path)
        |> String.replace("//input//", escape_gnuplot_string(dest_path_expanded))
        |> String.replace("//out_name//", escape_gnuplot_string(out_name))
        |> String.replace("//title//", escape_gnuplot_string(title))

      script_path = Path.expand(Path.join(base_tmp_dir, "gantt_script.plt"))
